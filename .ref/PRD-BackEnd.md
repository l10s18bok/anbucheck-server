...
- 보호자가 대상자를 식별하기 위한 별칭은 **클라이언트 로컬이 원본**이며, 서버에는 **Push 렌더링용 사본**을 보관한다(`guardians.alias`, §4.4.1). 알림 트레이에 "삼촌 · …"처럼 누구의 알림인지 표시하려면 앱이 꺼져 있어도 OS가 그리는 알림에 이름이 들어가야 하고, 그 문구는 서버만 만들 수 있기 때문이다. 이는 **대상자의 실명이 아니라 보호자가 스스로 붙인 호칭**이고, 저장한 그 보호자에게만 되돌려 보내진다(다른 보호자·대상자에게 노출되지 않음)
...
- **야간 억제는 보호자별 DND 하나로만 한다 (서버 전역 야간 발송 제한은 없음):**
  - 구현: `guardian_notification_settings`(`dnd_enabled`/`dnd_start`/`dnd_end`, 기본 **OFF**) → `alert_service.is_in_dnd` → `should_push`. 보호자 자기 타임존(`guardian_timezone`) 기준이며 자정을 넘기는 구간(예: `22:00~07:00`)도 처리한다.
  - **긴급(urgent)은 DND와 무관하게 항상 발송한다.**
  - DND에 걸리면 **푸시만 생략**되고 알림 자체는 사라지지 않는다 — `_save_notification_event`가 푸시 게이트보다 **먼저** 실행되므로 `notification_events`에 그대로 남아 보호자가 앱을 열면 in-app 알림 목록에서 확인할 수 있다.
  - ⚠️ **과거 스펙에 있던 "22:00~09:00 판정 경고를 익일 09:00으로 지연 발송"은 삭제됐다. 다시 넣지 말 것.** 최초 커밋부터 DND와 함께 존재했으나 한 번도 구현되지 않았고(`QUIET_HOUR_START/END` 상수는 정의만 되고 참조 0회 — 이번에 제거), 구현했다면 오히려 해로웠다:
    1. **긴급 등급까지 지연 대상이었다** — 새벽 02:00 판정 긴급 경고가 09:00에 도착하면 최대 11시간 지연이다. 독거 고령자 안부 확인 서비스에서 용납되지 않는다. DND가 urgent를 예외로 두는 현재 동작이 옳다.
    2. **고정 `22:00~09:00`의 기준 타임존이 정의 불가**다(보호자? 대상자?). 20개 언어 글로벌 서비스에서 의미가 없으며, DND는 `guardian_timezone`으로 이미 해결했다.
    3. **둘이 공존하면 상호작용이 정의 불가**다 — "익일 09:00 발송" 시점이 그 보호자의 DND 구간 안이면 어느 쪽이 이기는지 정할 방법이 없다.
  - 남은 한계(별건): DND 기본값이 OFF라 설정하지 않은 보호자는 늦은 시각(예약시각 상한 21:59 → 판정 최대 23:59) 푸시를 받을 수 있다. 해결한다면 DND 기본값/온보딩 쪽에서 다룰 문제이지, 두 번째 억제 메커니즘을 추가할 일이 아니다.
- 보호자가 heartbeat 시각을 변경하면 경고 발송 시각도 자동으로 조정됨

**경고 Push 메시지 (개인정보 미포함):**

> **모든 푸시 `data`에 `sent_at`(발송 시각, UTC epoch 초 문자열)이 붙는다** (2026-09-16, `push_service.send_push` 한 곳). 아래 예시에서는 생략했다. iOS 확장이 "APNs에 보관됐다가 **뒤늦게** 배달된 경고"를 가려내 그때만 안부를 얹어 보내는 데 쓴다(PRD-FrontEnd §2.2.1). 구버전 앱은 무시하며, 필드가 없으면 확장은 현행 동작(즉시 통과)으로 떨어진다. ⚠️ 이 키를 빼거나 형식(초 단위 정수)을 바꾸면 보호 대상자가 있는 G+S 아이폰이 오프라인 뒤 복구 전송을 못 하게 된다.
```json
// 정보 등급 — 배터리 방전 추정
{
...
{
  "notification": { "title": "✅ 정상 복귀", "body": "보호 대상자의 안부가 정상적으로 확인되었습니다." },
  "data": { "type": "alert_resolved", "subject_user_id": "1", "invite_code": "K7M-4PXR" }
}
```
- 서버에 실명이 없으므로 Push **본문**은 "대상자"로 표시
- Push **본문** 앞에는 보호자가 붙인 별칭이 붙는다 — `"오늘 안부 확인이…" → "삼촌 · 오늘 안부 확인이…"` (`push_service.decorate_body`, §6.7). 별칭이 없으면(미동기화·구버전 앱) 본문도 정형 문구 그대로라 하위호환이 유지된다
- 앱 내 화면(대시보드·알림 목록)은 여전히 **로컬 별칭**으로 표시한다 — 서버 사본은 Push 렌더링 전용이며 `/subjects` 응답에는 포함되지 않는다
...
보호자 Push에 **"누구의 알림인지"**를 표시하기 위해 서버가 별칭을 알아야 한다. 별칭의 **원본은 보호자 앱 로컬**이고 서버는 렌더링용 사본만 보관한다.
...
- 서버는 heartbeat 수신 시 해당 기기의 `last_seen`을 갱신한다 — **단 "오늘의 안부 확인"으로 분류된 기록만**(`is_todays_report`)

> ⚠️ **`last_seen`은 "오늘의 안부가 확인된 시각"이다.** 지난 기록 보정(`<지난날>_HH:MM`)과 살아있음 신호(`recovery_<날짜>`)는 이 값을 **전진시키지 않는다** (2026-09-09 수정, `services/heartbeat_service.py`).
>
> 전진시키면 스케줄러의 두 잡이 그 기기를 **그날 통째로 건너뛴다** — 둘 다 `last_seen < 오늘 로컬 자정`으로 대상을 거르기 때문이다:
> - `job_ios_heartbeat_trigger`(예약시각 정각) — iOS는 이것이 유일한 자동 전송 트리거라, 미발사되면 사용자가 앱을 다시 열지 않는 한 **그날 안부 경로가 통째로 사라진다**
...
>
> ⚠️ **살아있음 신호(`recovery_`)는 `last_seen`은 밀지 않지만 활성 경고는 해소한다** (2026-09-12 확정). 이 둘은 별개의 사실이다 — `last_seen`을 밀면 그날 트리거와 미수신 체크가 그 기기를 통째로 건너뛰지만, 경고 해소는 그런 부작용이 없다(`alerts`는 독립 테이블).
>
> **해소하는 이유 둘.** ① 증거의 양이 정상 전송과 같다 — 해소는 `suspicious=false` 분기에서만 일어나고, 회복 전송이 나가는 조건(`걸음수>0 || 잠금해제 || 오늘앱실행`)은 정상 전송이 `suspicious=false`가 되는 조건과 **같은 세 신호**다. 빠진 것은 "예약시각에"뿐이다. ② 죽어 있던 기기가 살아났다는 사실은 **한시라도 빨리** 보호자에게 닿아야 한다 — 보호자가 헛되이 전화하거나 찾아가는 것을 막는 것이 이 신호의 존재 이유다. 알리지 않으면 매일 "안부 확인 완료"는 정상으로 가는데 대시보드 카드만 경고로 남는 불일치도 굳는다.
>
> **대가(알고 받아들인다)**: 회복 전송이 도착한 날 **그날 정시 전송까지 실패하면** 그 날짜의 등급이 한 칸 오르지 않는다(주의 → 주의). 영구 상한이 아니라 그만큼 지연이다 — 기기가 완전히 죽으면 회복 전송도 멈춰 사다리가 정상적으로 오르고, 정시 전송이 성공한 날은 어차피 그쪽이 해소하므로 결과가 같다.
>
> ⚠️ 2026-09-10에 이것을 `is_todays_report`로 게이팅했다가 09-12에 되돌렸다. 당시 근거였던 **"사다리가 매일 리셋된다"는 과장**이었다 — 미수신 사다리는 `alerts` **행**으로 오르므로(`scheduler.py`의 `has_active_alert`), 정시 전송이 성공한 날에는 그 전송이 어차피 경고를 지운다. 같은 근거로 다시 게이팅하지 말 것.
>
> ⚠️ **`suspicious_count`도 같이 묶인다** — 한쪽만 게이팅하면 "경고는 지워졌는데 카운터는 2"가 되어 다음 suspicious 한 번에 곧장 상위 등급으로 튄다. 두 처리는 **반드시 함께** 움직인다.
>
> ⚠️ **SOS(`note='emergency_request'`)도 함께 해소된다**(`include_emergency=True`). 자동 경로를 `False`로 바꾸지 말 것 — 사람 흔적이 확인된 분기에서만 도달하므로 "쓰러진 사람 옆에서 폰이 혼자 SOS를 지우는" 경로는 없고, SOS 푸시는 이미 즉시·무조건 전 보호자에게 나갔으며 알림 목록에도 그날 내내 남는다. 여기서 지우는 것은 사건이 아니라 카드 등급이다. 막으면 SOS 행을 지우는 경로가 보호자의 [건강 확인 완료]뿐이 되어, 아무도 누르지 않을 때 카드가 **영구히 긴급**으로 박힌다(자정 정리도 만료도 없다). 단 **지난 기록 보정(backfill)은 `False`** — 그건 "그 날 살아 있었다"는 사후 증거라 지금의 SOS 상태를 말해 주지 못한다.
...
CREATE TABLE IF NOT EXISTS guardians (
    ...
    alias               TEXT,
                        ...
                        -- Push 본문에 "누구의 알림인지"를 표시하는 용도로만 쓴다.
...
### 6.0.1 Android 조기 미전송 알림 (예약시각 **+60분**, 매 1분 체크)

> `services/scheduler.py: job_early_safety_net` / 락 `LOCK_EARLY_SAFETY_NET=7` / job id `early_safety_net`
> 2026-09-13 +30m 사일런트로 도입 → **2026-09-14 +60m 표시형으로 교체**

**이 잡은 미전송을 "해결"하지 않는다. 막혀 있던 것을 "뚫는다."**

삼성 등 일부 Android 기기는 `RUN_ANY_IN_BACKGROUND` 앱옵을 `ignore`로 돌려 패키지 단위로
백그라운드 실행을 차단한다. 0차 알람과 1차 WorkManager job이 **한꺼번에** 막히고, 문을 여는 것은
**고우선순위 FCM이 받는 temp-power-save allowlist**(약 20초)뿐이다(실측: 앱 repo
`android_scheduling_field_notes.md` §8). 진입은 마지막 포그라운드 실행 +약 3일, 해제는 포그라운드
실행 1회라 **앱을 열지 않는 순수 대상자에게만** 선택적으로 걸린다.

지금까지 문을 연 것은 §6.1의 `subject_safety_net`(+2h)인데 **그 tick이 보호자 경고를 만드는 tick**이라
안부가 7~14초 뒤 도착하는데도 매일 미수신 판정이 먼저 났다. 이 잡이 **같은 알림을 1시간 먼저** 보낸다.

**발송 조건** — §6.1 쿼리의 복사본이며 차이는 **`+120` → `+60`**과 **`AND d.platform = 'android'`**뿐:
...
WHERE u.invite_code IS NOT NULL
  ...
  AND date_trunc('minute', now()) = date_trunc('minute',
        (date_trunc('day', now() AT TIME ZONE zz.tz)
           + make_interval(mins => d.heartbeat_hour * 60 + d.heartbeat_minute + 60)
        ) AT TIME ZONE zz.tz)
  AND d.last_seen < (date_trunc('day', now() AT TIME ZONE zz.tz) AT TIME ZONE zz.tz)
```

⚠️⚠️ **platform 필터는 필수다.** §6.1 쿼리에는 platform 필터가 없고 Android 게이팅이
`_process_missed_heartbeat` **안**에 있다. 복사할 때 빠뜨리면 iOS 대상자에게 가서 **APNs 보관 슬롯
(앱당 1칸)을 빼앗아 §6.0의 그날 트리거를 소실**시킨다(`ios_nse_field_notes.md` §13.5).

**푸시 내용 — `push_subject_safety_net`을 그대로 재사용한다(새 함수 금지):**

| 항목 | 값 | 이유 |
|---|---|---|
| 제목/본문 | `push_subject_safety_net_title/body` (+2h와 동일) | +60분 시점에도 "아직 안 나갔다"가 참 |
| `data.type` | `subject_safety_net` | 탭 라우팅이 기존과 동일(safety_home) |
| tag | `anbu_safety_net` | 안부 전송 성공 시 앱이 이 태그로 **알림을 지운다**(실측 1.7초). +2h 알림이 또 가도 **교체**되어 쌓이지 않음 |

⚠️⚠️ **데이터 전용(사일런트) 푸시로 되돌리지 말 것.** 09-13에 +30m 사일런트로 배포했다가 다음 날
교체했다. FCM 문서: *"If FCM detects a pattern in which messages don't result in user-facing
notifications, your messages may be deprioritized to normal priority"* / *"When the device is in Doze
mode, delivery may be delayed"*. ⚠️ 실측 근거는 아직 없다 — 09-14 A66-Q1OR(09-06~09-10 매일
20:00:12~14 전송)이 18:30 사일런트에 무반응이었으나 **같은 날 20:00 표시형에도 무반응**(기기 도달
불가)이라 판정 불가였다. 교체는 **문서 근거 + 표시형이 문을 연다는 실측**(삼성폰 9일)에 기댄다. 게다가 "your messages"라 **앱 전체 메시지가 강등될 위험**이 있어, 효과 없는 사일런트가 유일하게 문을
여는 +2h 알림까지 해칠 수 있었다.

⚠️ **왜 +60분인가** — allow-while-idle 알람의 배달 창이 **[예약시각, +60분]**이다(`window=+1h0m0s`).
+30분이면 알람이 창 후반(실측 +29~32분)에 뜨는 **정상 기기까지** 매일 알림음을 듣는다(09-13 테스트폰은
+30분 20초 전에 보냈다). +60분이면 정상 기기는 이미 보낸 뒤라 **막힌 기기만** 받는다.

⚠️ **소리·진동이 난다.** 안드로이드 8+는 소리를 서버가 아니라 **앱의 알림 채널**이 정하고, 앱 채널은
`anbu_alerts`(Importance.high) 하나뿐이다. 서버가 없는 채널 ID를 지정하면 FCM이 기본 채널로 폴백한다.
조용히 하려면 **앱에 저중요도 채널을 추가**해야 한다(미착수). 막힌 기기는 원래 +2h에 같은 알림을
받았으므로 **새 부담이 아니라 시각이 당겨지는 것**이며, +2h 알림·보호자 미수신 경고가 대신 사라진다.

**§6.1(+2h)과의 관계**: 함수·쿼리·락·job id 전부 독립이고 `job_heartbeat_check`는 무변경.
끄려면 `add_job` 한 줄만 주석 처리. 뚫리면 `last_seen`이 오늘이 되어 §6.1이 자동 제외, 못 뚫리면
§6.1이 기존대로 — **최악이 현재 동작이다.** 보호자 경고는 만들지 않는다.

**부하**: 대상은 +60분까지 미전송인 부분집합, 루프 본문 FCM 1회, DB 쓰기 없음 → `asyncio.gather` +
세마포어(20) 동시 발송, 커넥션은 gather 전 반납. 요약 로그 `[조기 미전송 알림] 발송 — 대상 N건 / 성공 N건 /
Nms`, 기기별로는 `send_push`가 `subject_safety_net (토큰 앞 10자)`를 남긴다.
...
2. 대상자 본인 안부유도 푸시 (Android 한정 — 구독·보호자 유무와 무관):
   ...
   - ⚠️ **이 푸시가 "LAST-RESORT"가 아니라 그날의 유일한 전송 트리거가 되는 기기 상태가
     ...
     1시간 앞서 같은 알림을 보내는 §6.0.1을 따로 뒀다.
...
**Push 본문 렌더링**: `push_service.decorate_body(body, alias)`가 `"오늘 안부 확인이…" → "삼촌 · 오늘 안부 확인이…"`처럼 **본문 앞에** 덧붙인다. 이 함수는 모든 보호자 알림이 지나는 단일 통로 앞단이므로 **어떤 입력에도 예외를 던지지 않는다** — `alias`가 NULL·빈 문자열·비문자열이면 원본 본문을 그대로 반환한다. 여기서 예외가 나면 경고·긴급 Push 전체가 죽는다.

⚠️ **제목이 아니라 본문인 이유 (실기기 확인, 2026-08-09)**: 접힌 알림에서 제목은 어떤 기기에서도 줄바꿈되지 않고, 샤오미(MIUI/HyperOS)는 제목과 본문을 **한 줄에 이어붙여** 표시해 긴 제목(`✅ 오늘 안부 확인 완료`) 뒤의 별칭이 먼저 잘린다. 펼친 알림에서 여러 줄로 늘어나는 건 본문뿐이라 본문 앞에 둬야 20자가 온전히 읽힌다. **제목·본문 양쪽에 넣지 말 것** — 한 줄에 별칭이 두 번 나온다.

⚠️ **별칭은 문장 *바깥*에 구분자로만 붙인다** — 문장 안에 끼워 넣으면(`"삼촌의 안부 확인이…"`) 한국어 주격조사 이/가, 러시아어·폴란드어 격변화, 터키어 소유격 모음조화, 아랍어 정관사가 걸리는데 별칭은 자유 입력이라 규칙을 세울 수 없다. 구분자를 ` · `로 고정한 것도 같은 이유(쉼표·콜론은 로케일별 조판 규칙이 있음). 덕분에 20개 언어 문구는 무수정이다.
...
⚠️ **발음 부호(diacritic)를 제거하지 말 것.** 과거 라틴 문자권 문자열이 ASCII로 치환돼 있었는데, 관례가 아니라 손상이었다(같은 로케일 안에서 부호가 있는 문자열과 없는 문자열이 뒤섞여 있었고, 스웨덴어는 `ö→oe`와 `ö→o`를 혼용했다). 단순 미관 문제가 아니라 **문장이 깨진다**:

| 언어 | 손상 | 결과 |
|---|---|---|
| 이탈리아어 | `è`(이다) → `e`(그리고) | 문장에서 동사가 사라짐 — `La batteria e inferiore al 20%` |
| 프랑스어 | `à`(~에) → `a`(가지다) | 전치사가 동사가 됨 |
| 스페인어·포르투갈어 | `está`(이다) → `esta`(이것) | 동사가 지시대명사가 됨 |
| 베트남어 | 성조 부호 전체 | 성조가 의미를 결정하는 언어라 판독성 급락 |

UTF-8로 정상 저장·전송되므로 인코딩 제약도 없다. 2026-08-10에 전 로케일 복원 완료(커밋 `66c3d09`).

**용어 일관성**: 한 로케일 안에서 같은 개념에 여러 어휘를 쓰지 말 것. 중국어가 `安否`·`健康确认`·`问安`·`安全确认` 네 가지를 혼용하고 있었고(`安否`는 《예기》 출전 문언체라 현대 중국어에 부적합), `平安`으로 통일했다. 독일어는 `Wohlstand`(부유함·번영)가 `Wohlbefinden`(안녕) 자리에 8곳 들어가 있었다.
...
| `/api/v1/subjects/aliases` | PUT | 대상자 별칭 동기화 (보호자용, Push 본문 표시용) |
