...
>
> ⚠️ **미해결(별건) — recovery가 경고 등급 사다리를 매일 리셋한다.** recovery는 `resolve_active_alerts`로 활성 경고를 전부 지우고 `suspicious_count`도 0으로 만든다. 그리고 구조적으로 그날 미수신 체크(+2h)보다 항상 먼저 온다. 그래서 연속 미전송이어도 매일 "주의"만 반복되고 경고·긴급으로 올라가지 못한다. 이 수정의 범위 밖이며 별도로 다룬다.
