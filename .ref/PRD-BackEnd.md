...
>
> ⚠️ **살아있음 신호(`recovery_`)는 활성 경고를 해소하지 않는다** (2026-09-10 수정, 위 `last_seen` 규칙의 후속). recovery가 주장하는 것은 "이 기기가 켜져 있다"이지 "오늘의 안부가 확인됐다"가 아니다. 그런데 recovery는 구조적으로 **그날 미수신 체크(+2h)보다 항상 먼저** 온다(예약시각 이전에만 발동하므로). 그래서 `resolve_active_alerts`로 경고를 지우면 다음이 된다 — 1일차 저녁 "주의" → 2일차 아침 recovery가 경고 삭제 → 저녁에 다시 "주의" → 3일차도 "주의"…. **며칠째 안부가 없는데 주의에서 못 올라간다** = 경고·긴급 에스컬레이션 사다리가 무력화된다.
>
> 대가는 "기기가 살아났는데 보호자 경고가 몇 시간 더 남는다"이며, 그날 예약시각 정시 전송이 성공하면 그때 정상 해소된다. 정시 전송까지 실패하면 경고가 유지된 채 사다리가 올라간다(의도한 동작).
>
> ⚠️ **`suspicious_count`도 같이 묶인다** — 경고만 남기고 카운터를 0으로 되돌리면 "경고는 caution인데 카운터는 0"이라는 불일치가 생겨 사다리가 여전히 안 올라간다. 두 게이팅은 **반드시 함께** 유지한다.
>
> ⚠️ **지난 기록 보정(backfill)은 예외다** — 경고 해소도 하고 카운터도 리셋한다. 그건 그 날 기기가 실제로 살아 있었다는 **사후 증거**라 성격이 다르다.
